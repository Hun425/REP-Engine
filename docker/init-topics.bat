...
docker exec %KAFKA_CONTAINER% kafka-topics --create --bootstrap-server localhost:9092 --topic product.inventory.v1.dlq --partitions 3 --replication-factor 1 --config retention.ms=2592000000 --if-not-exists

docker exec %KAFKA_CONTAINER% kafka-topics --create --bootstrap-server localhost:9092 --topic notification.push.v1.dlq --partitions 3 --replication-factor 1 --config retention.ms=2592000000 --if-not-exists
