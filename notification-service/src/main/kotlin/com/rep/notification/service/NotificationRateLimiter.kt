...
class NotificationRateLimiter(
    ...
    private val duplicateBlockedCounter =
        Counter
            .builder("notification.rate.duplicate.blocked")
            .description("Notifications blocked due to duplicate")
            .register(meterRegistry)

    private val dailyLimitBlockedCounter =
        Counter
            .builder("notification.rate.daily.blocked")
            .description("Notifications blocked due to daily limit")
            .register(meterRegistry)

    private val failCloseBlockedCounter =
        Counter
            .builder("notification.rate.fail_close.blocked")
    ...
    suspend fun shouldSend(
        ...
        return try {
            ...
            if (!wasSet) {
                log.debug {
                    "Duplicate notification blocked: userId=$userId, productId=$productId, type=$notificationType"
                }
    ...
    suspend fun canSend(
        ...
        productId: String,
        notificationType: String,
    ): Boolean = checkDailyLimit(userId) && shouldSend(userId, productId, notificationType)
